...
for movingImage in `ls ${input}/*T2w*.nii.gz`;
    ...
    #movingImage=${file}
    fixedImage=/home/k2252514/templates/nihpd/nihpd_asym_02-05_t2w.nii
    antsRegistrationSyN.sh -d 3 -f ${fixedImage} -m ${movingImage} -t r	-o ${input}/tmp/reg-${name}
done

# Trasform planes to isotropic image
cd ${input}/tmp
antsMultivariateTemplateConstruction2.sh -d 3 -i 1 -r 1 -k 1 -f 4x2x1 -s 2x1x0vox -q 30x20x4 -t SyN -m MI -c 0 -o MY reg-site*T2w*.nii.gz
cp ${input}/tmp/MYtemplate0.nii.gz ${input}/${3}_T2w_iso.nii.gz
rm -r ${input}/tmp/
